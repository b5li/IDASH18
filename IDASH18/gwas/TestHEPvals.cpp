...
void TestHEPvals::testHELinReg(double*& zScore, double*& pVals, double* yData, double** xData, double** sData, long factorDim, long sampleDim, long nsnp,  string filename){
    ...
    SecretKey secretKey(context, 1);   // binary
...
void TestHEPvals::testHESIMDLinReg(double*& zScore, double*& pVals, double* yData, double** xData, double** sData, long factorDim, long sampleDim, long nsnp,  string filename){
    ...
    long logN = 13;
    long nslots = (1<< (logN-1)); //! total number of plaintext slots
    long L = 4;     // 3 (for det), 1 (for update)
    long K = L + 1; //! need for "correct" rotation (why?)

    //! Parameters for GWAS
    long sampleDim2 = (1 << (long)ceil(log2(sampleDim)));
    long log2sampleDim = (long)ceil(log2(sampleDim)); // logw(sampleDim)
    
    long nXbatching = nslots/(sampleDim2 * factorDim);  // replicated number of a user's data in a single ciphertext
    long nCovbatching = nslots/(sampleDim2 * 8);               // 2^12/(2^8 * 8)
    ...
    cipherPvals.encryptSIMDXData(encYXData, enccovData, yData, xData, factorDim, sampleDim, sampleDim2, nXbatching, nCovbatching, dim, nterms, scalefactor, nslots, L) ;
    ...
    //! 24 Rot
    Ciphertext* encYX = new Ciphertext[factorDim];
    cipherPvals.aggYXData(encYX, encYXData, log2sampleDim, nXbatching, factorDim, nslots);
    
    //! (34 * 8) Rot
    Ciphertext* encCov = new Ciphertext[34];
    cipherPvals.aggCovData(encCov, enccovData, log2sampleDim, nCovbatching);
    ...
    cipherPvals.encSIMDAdjoint_smalldep(encDet, encAdj, encCov);
