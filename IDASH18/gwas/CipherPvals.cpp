...
//! nterms = 10
void CipherPvals::computeCov(double*& covariance, double* data, long dim, long scalefactor){
    long k = 0;
...
void CipherPvals::encryptSIMDXData(Ciphertext& encYXData, Ciphertext*& enccovData, double* yData, double** xData, long factorDim, long sampleDim, long sample2, long nXbatching, long nCovbatching, long dim, long nterms, long scalefactor, long nslots, long L) {
    ...
    for (long i = first; i < last; ++i) {
        ...
        computeCov(cov[i], xiData[i], dim, scalefactor);  //! dim -> dim * (dim-1)/2
    }
    NTL_EXEC_RANGE_END;
...
/********************************************************************/
//! encYX[k] = YXvec = y^T * X[k] = [-29, -6.506140997, -6.368385672, -12.88024343] (0 <= k < 4)
//! 8 Rot + 16 Rot
void CipherPvals::aggYXData(Ciphertext*& encYX, Ciphertext encYXData, long log2sampleDim, long nbatching, long factorDim, long nslots){
    long nslots1 = nbatching * factorDim;
    Ciphertext res = encYXData;
    
    //! Allsum
    for(long i = 0; i < log2sampleDim; ++i){
...
//! (34 * 8) Rot 
void CipherPvals::aggCovData(Ciphertext*& encCov, Ciphertext* enccovData,  long log2sampleDim, long nbatching){
...
//!@ Full replication of vector of size 4
...
void CipherPvals::encSIMDAdjoint(Ciphertext& encDet, Ciphertext*& encAdj, Ciphertext* encData){
    ...
    for (long l = first; l < last; ++l){
        ...
        encAdj[l] = extscheme.keySwitch(temp);
    ...
    encDet = extscheme.keySwitch(temp[0]);
...
void CipherPvals::encSIMDAdjoint_smalldep(Ciphertext& encDet, Ciphertext*& encAdj, Ciphertext* encData){
    ...
    for (long l = first; l < last; ++l){
        ...
        scheme.modDownToAndEqual(encData[3 * l + 2], encAdj[l].l);
        scheme.multAndEqual(encAdj[l], encData[3 * l + 2]);
    ...
    for (long l = first; l < last; ++l){
        ...
        encAdj[l + 4] = extscheme.keySwitch(temp);
...
void CipherPvals::extQuadForm(Ciphertext& res, Ciphertext* encData1, Ciphertext* encMatrix, Ciphertext* encData2, long factorDim){
    ...
    for(long i = first; i< last; ++i){
        ...
        for(long j = 0; j < i; ++j){
            ...
            //ctemp1[i][j] = scheme.modDownTo(encData1[i], encMatrix[l1].l);
            //ctemp2[i][j] = scheme.modDownTo(encData2[j], encMatrix[l1].l);
            //tensoring[i][j] = extscheme.rawmult3(ctemp1[i][j], ctemp2[i][j], encMatrix[l1]);
            // In the final step, rescaling by "2"
    ...
    //! Final KS rescaling
    res = extscheme.keySwitch(extres);
...
void CipherPvals::extencAdjoint(Ciphertext& encDet, Ciphertext*& encAdj, Ciphertext* encData){
    ...
    NTL_EXEC_RANGE(10, first, last);
    for(long i = first; i < last; ++i){
        encAdj[i] = extscheme.keySwitch(extAdj[i]);
        scheme.reScaleByAndEqual(encAdj[i], 2);
    }
    NTL_EXEC_RANGE_END;
    ...
    encDet = extscheme.keySwitch(encDetemp[0]);
    scheme.reScaleByAndEqual(encDet, 1);
