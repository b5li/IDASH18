...
void CipherLRPvals::encVecSData(Ciphertext* encSWS, Ciphertext** encSWX, Ciphertext encWData, Ciphertext** encSData, Ciphertext*** encSXData, uint64_t** poly0, uint64_t** poly, long factorDim, long sampleDim, long nencsnp, long nslots, long subblocksize, long niter, long nstep, long nblock, long* rot){
    ...
    for (long i = first; i < last; ++i) {
        ...
        if(i != niter1){
            ...
            for(long j = 0; j < nencsnp; ++j){
                scheme.modDownToAndEqual(tmp[0], encSData[i1][j].l);
                res[i][j] = extscheme.rawmult(encSData[i1][j], tmp[0]);
                
                for(long l = 1; l < subblocksize; ++l){
                    Ciphertext tmp1 = scheme.modDownTo(tmp[l], encSData[i1 + l][j].l);
                    ExtCiphertext extmp = extscheme.rawmult(tmp1, encSData[i1 + l][j]);
                    extscheme.addAndEqual(res[i][j], extmp);
                }
            }
        }
    ...
    for(long j = 0; j < nencsnp; ++j){
        ...
        extscheme.reScaleByAndEqual(res[0][j], 1);
        encSWS[j] = extscheme.DecompKeySwitchMT(res[0][j]);
