...
// g++ -std=c++11 -O3 -I/usr/local/include  -pthread main.cpp  -o foo -L../lib -L/usr/local/lib -lHEAAN -lntl -lgmp -lm
using namespace std;
...
void ExtTestScheme::testBasic(long logN,  long L, long logp, long logSlots){
    ...
    TimeUtils timeutils;
    long K = L;
    Context context(logN, logp, L, K, 1);
...
void ExtTestScheme::testThreeProd(long logN,  long L, long logp, long logSlots){
    ...
    Ciphertext multCipher = extscheme.keySwitch(extmultCipher);
    ...
    Ciphertext multCipher0 = extscheme.keySwitch(extmultCipher0);
    ...
    Ciphertext multCipher1 = extscheme.keySwitch(extmultCipher2);
    ...
    Ciphertext sqrCipher = extscheme.keySwitch(extsqrCiphext);
...
void ExtTestScheme::testDecompKS(long logN,  long L, long logp, long logSlots){
    ...
    //Ciphertext multCipher1 = extscheme.keySwitch(extmultCipher1);
    Ciphertext multCipher1 = extscheme.decompKeySwitch(extmultCipher1);
    timeutils.stop("Homomorphic Key Switching");
    
    scheme.reScaleByAndEqual(multCipher1, 1);
    
    complex<double>* dvecMult = scheme.decrypt(secretKey, multCipher1);
    StringUtils::showcompare(mvecMult, dvecMult, slots, "raw0");
   
    
}
void ExtTestScheme::testDecompRotate(long logN,  long L, long logRotSlots, long logp, long logSlots){
    ...
    for(long l = 0; l < slots; ++l){
        cout << mvec[l].real() << endl;
    }
    
    Ciphertext cipher = scheme.encrypt(mvec, slots, L);
    
    Ciphertext rotcipher = extscheme.leftRotateFast(cipher, 1 << logRotSlots);
    
    complex<double>* dvec = scheme.decrypt(secretKey, rotcipher);
