...
//! cipher - cnst
Ciphertext Scheme::subConst(Ciphertext& cipher, double cnst) {
    uint64_t tmpr = abs(cnst) * context.p;
    uint64_t* ax = new uint64_t[cipher.l << context.logN]();
    uint64_t* bx = new uint64_t[cipher.l << context.logN]();
    copy(cipher.ax, cipher.ax + (cipher.l << context.logN), ax);
...
//! cnst - cipher
Ciphertext Scheme::subConst2(Ciphertext& cipher, double cnst) {
    uint64_t tmpr = abs(cnst) * context.p;
    uint64_t* axres = new uint64_t[cipher.l << context.logN]();
    uint64_t* bxres = new uint64_t[cipher.l << context.logN]();
    ...
    if(cnst >= 0) {
        context.subConst(bxres, cipher.bx, tmpr, cipher.l);
    } else {
        context.addConst(bxres, cipher.bx, tmpr, cipher.l);
    }
    context.negateAndEqual(bxres, cipher.l);
