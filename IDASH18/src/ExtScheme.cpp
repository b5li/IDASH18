...
ExtScheme::ExtScheme(SecretKey& secretKey, Context& context, Scheme& scheme) : context(context), scheme(scheme) {
	addThreeMultKey(secretKey);   // Ps^3 -> s
    
}

void ExtScheme::addDecompTwoKeys(SecretKey& secretKey){
    uint64_t** Keyax = new uint64_t*[context.L];
    uint64_t** Keybx = new uint64_t*[context.L];
    ...
    for(long i = 0; i < context.L; ++i){
        Keyax[i] = new uint64_t[(context.L + 1) << context.logN];
        Keybx[i] = new uint64_t[(context.L + 1) << context.logN];
        ...
        context.sampleUniform(Keyax[i], context.L, 1);
        context.mul(Keybx[i], Keyax[i], secretKey.sx, context.L, 1);
        context.sub2AndEqual(ex, Keybx[i], context.L, 1);
        
        decompTwoKeyMap.insert(pair<long, Key>(i, Key(Keyax[i], Keybx[i])));
    }
    delete[] ex;
...
void ExtScheme::addDecompThreeKeys(SecretKey& secretKey){
    ...
    uint64_t* p0sx2i = new uint64_t[context.L << context.logN];
 
    // Compute pVec[0] * s^3
    context.mul(p0sx3, secretKey.sx, secretKey.sx, context.L);
    context.mulAndEqual(p0sx3, secretKey.sx, context.L);
    context.mulConstAndEqual(p0sx2i, context.pVec[0], context.L);
    ...
    for(long i = 0; i < context.L; ++i){
        ...
        decompThreeKeyMap.insert(pair<long, Key>(i, Key(Keyax[i], Keybx[i])));
    }
    delete[] ex;
}
...
void ExtScheme::addDecompLeftRotKey(SecretKey& secretKey, long rot){
    ...
    for(long i = 0; i < context.L; ++i){
        ...
        decompLeftRotKeyMap.insert(pair< pair<long, long>, Key> (make_pair(rot, i), Key(Keyax[i], Keybx[i])));
    }
    delete[] ex;
}

//! Generate all the rotation key
...
Ciphertext ExtScheme::decompKeySwitch(ExtCiphertext& cipher) {
    ...
    uint64_t* tmpj = new uint64_t[(cipher.l + 1) << context.logN];
    ...
    if(cipher.deg > 1){
        for(long i = 0; i < cipher.l; ++i){
            ...
            uint64_t* axi = cipher.ax[1] + (i << context.logN);
            
            // Generate RNS(axi) mod p_0, q_l
            context.qiINTTAndEqual(axi, i);
            
            for(long j = 0; j < cipher.l; ++j){
                tmpj = tmp + (j << context.logN);
                context.qiNTT(tmpj, axi, j);
            }
            context.piNTT(tmp + (cipher.l << context.logN), axi, 0);
            
            context.mul(axtmp, keys.ax, tmp, cipher.l, 1);
            context.mul(bxtmp, keys.bx, tmp, cipher.l, 1);
    ...
    if(cipher.deg > 2){
        for(long i = 0; i < cipher.l; ++i){
            ...
            uint64_t* axi = cipher.ax[2] + (i << context.logN);
    ...
    for(long i = 0; i < cipher.l; ++i){
        uint64_t* axtmpi = axtmp + (i << context.logN);
        uint64_t* bxtmpi = bxtmp + (i << context.logN);
    ...
    for(long i = 0; i < cipher.l; ++i){
        ...
        uint64_t p0InvModqi = invMod(context.pVec[0], context.qVec[i]);
...
Ciphertext ExtScheme::leftRotateFast(Ciphertext& cipher, long rotSlots){
    ...
    uint64_t* ax = new uint64_t[(cipher.l + 1) << context.logN]();
    ...
    for(long i = 0; i < cipher.l; ++i){
        ...
        uint64_t* axi = bx + (i << context.logN);
    ...
    // sumup
    context.addAndEqual(bxmult, bxrot, cipher.l);
