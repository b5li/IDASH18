...
void CipherLRPvals::new_encSData(Ciphertext***& encSXData, double** xData, double** sData,  long factorDim, long sampleDim, long nsnp, long nencsnp, long nslots, long SXlvl) {
    long nslots1 = nsnp - (nencsnp-1) * nslots;   // number of slots in the final ctxts
...
/****************************************************************************************************************/
//! Output: encWData
//! w[t+1] <- v[t] - gamma * g(v[t])
//! v[t+1] <- (1-eta) * w[t+1] + eta * w[t]

void CipherLRPvals::encNLGD(Ciphertext& encWData, Ciphertext encYXData, long factorDim,  long sampleDim, long fdimBits, long sdimBits, long xBatchingBits, uint64_t* poly, long numIter, double gammaUp){
    ...
    double gamma = (double) gammaUp / sampleDim;
    
    if(numIter == 1){
        encNLGD_oneiter(encWData, encYXData, sdimBits, xBatchingBits, gamma);
    }
    else{
        //! 0th iteration (1 lvl)
        Ciphertext* encW0Data = new Ciphertext[2];   //! sum gamma * 0.5 * XData
        Ciphertext encSumYX;                        //! encGrad = sum (YXData[i]/4)
        encNLGDiteration0(encW0Data, encSumYX, encYXData, sdimBits, xBatchingBits, gamma, eta[0]);
        
        //! 1th iteration (2 lvl)
        encWData = encW0Data[0];
        Ciphertext encVData = encW0Data[1];
        Ciphertext encScaledYX = extscheme.multByConstMT(encYXData, gamma * scaledsigmoid3[2]);       //! a[2] * x
        scheme.reScaleByAndEqual(encScaledYX, 1);
        
        if(numIter > 2){
            for (long iter = 1; iter < numIter - 1 ; ++iter) {
                if(abs(eta[iter]) < 0.00001){
                    encNLGDiteration_eta0(encWData, encVData, encYXData, encW0Data[0], encScaledYX, poly, fdimBits, sdimBits, xBatchingBits, gamma);
                }
                else{
                    encNLGDiteration(encWData, encVData, encYXData, encW0Data[0], encSumYX, encScaledYX, poly, fdimBits, sdimBits, xBatchingBits, gamma, eta[iter]);
                    cout << iter << endl;
                }
            }
        }
        
        //! final iteration: only update "wData"
        encNLGDiterationW(encWData, encVData, encYXData, encW0Data[0], encScaledYX, poly, fdimBits, sdimBits, xBatchingBits, gamma);
...
//!@ only one iteration for GD

void CipherLRPvals::encNLGD_oneiter(Ciphertext& encWData, Ciphertext encYXData, long sdimBits, long xBatchingBits, double gamma){
    Ciphertext encGrad = encYXData;
    for (long l = xBatchingBits; l < xBatchingBits + sdimBits; ++l) {
        Ciphertext tmp = extscheme.leftRotateFastMT(encGrad, (1 << l));
        scheme.addAndEqual(encGrad, tmp);
    }
    encWData = extscheme.multByConstMT(encGrad, -(gamma * scaledsigmoid3[0]));
...
//! suppose that eta ~ 0
void CipherLRPvals::encNLGDiteration_eta0(Ciphertext& encWData, Ciphertext& encVData, Ciphertext encYXData, Ciphertext encW0Data, Ciphertext encGrad2, uint64_t* poly, long fdimBits, long sdimBits, long xBatchingBits, double gamma){
    ...
    /*******************************************************/
    //! 2. evaluate  "encGrad = sum sig(encIP) * Z[i]" for 0 <= i < n, (w.lvl - 4)  = gamma * x * (a[0] + a[1] * IP + a[2] * IP^3)
...
//! suppose that eta ~ 0
void CipherLRPvals::encNLGDiteration(Ciphertext& encWData, Ciphertext& encVData, Ciphertext encYXData, Ciphertext encW0Data, Ciphertext encGrad0, Ciphertext encGrad2, uint64_t* poly, long fdimBits, long sdimBits, long xBatchingBits, double gamma, double eta){
...
//!@ Function: the final update of NLGD
//!@ Output: encWData (w.lvl - 4), encVData (w.lvl - 5): not updated
void CipherLRPvals::encNLGDiterationW(Ciphertext& encWData, Ciphertext encVData, Ciphertext encYXData, Ciphertext encW0Data, Ciphertext encGrad2, uint64_t* poly, long fdimBits, long sdimBits, long xBatchingBits, double gamma){
...
//!@ Function: update the weight
//!@ Output: encWData = Enc(W), encZWData = Enc(ZW)
...
//!@ Input: encSData, encCData
//!@        niter = (n/subblocksize), nstep = log2(subblocksize),
//!@ Output: encres[nencsnp] = Enc(s1^T * Vec, ...., sp^T * Vec)
//!@         e.g. encSWZ = Enc(s1^T * Wz, ...., sp^T * WZ) where (sj^T * WZ) = sum_i s[i][j] * (Wz[i])
//!@ Function: Firstly replicate (encCData), raw-multiply, RS and KS

void CipherLRPvals::encVecSData(Ciphertext*& encRes, Ciphertext encVecData, Ciphertext** encSData, uint64_t** poly0, uint64_t** poly, long sampleDim, long nencsnp,  long nslots, long subblocksize, long niter, long nstep, long nblock, long* rot){
    ...
    //! perform replication on each small block of size "("subblocksize")"
    ...
    for (long i = first; i < last; ++i) {
        ...
        if(i != niter1){
            ...
            fullReplicate(tmp, tmp[0], poly, rot, subblocksize, nstep);
        ...
        //! final sub-block
            ...
            sparseReplicate(tmp, tmp[0], poly, nslots, nvals, rot, subblocksize, nstep);
    ...
    for(long j = 0; j < nencsnp; ++j){
        ...
        encRes[j] = extscheme.DecompKeySwitchMT(res[0][j]);
    }
    //delete[] tmp;
    delete[] res;
}
...
//! encRes[nencsnp][k]
void CipherLRPvals::encVecSXData(Ciphertext**& encRes, Ciphertext encVecData, Ciphertext*** encSXData, uint64_t** poly0, uint64_t** poly, long factorDim, long sampleDim, long nencsnp, long nslots, long subblocksize, long niter, long nstep, long nblock, long* rot){
    ...
    //! Aggregate sum (res[i] * S[i])
    for(long j = 0; j < nencsnp; ++j){
        encRes[j] = new Ciphertext[factorDim];
        ...
        for (long k = first; k < last; ++k) {
            ...
            encRes[j][k] = extscheme.DecompKeySwitchMT(res1[0][j][k]);
        }
        NTL_EXEC_RANGE_END
    }
    //delete[] tmp;
...
//!@ (will be used with encW, so the input encoding level should be larger than encW.level)
//! poly0: 1st sub-block
//! poly1: 2nd sub-blocks

void CipherLRPvals::generateRepAuxPoly(uint64_t**& poly0, uint64_t**& poly, long nslots, long niter, long nstep, long* nblock, long* rot, long polylvl){
...
//!@ Input: Enc(z[0], z[1], ..., z[bsize-1]), nstep = log2(subblocksize)
//!@ Output: {Enc(z[i]}, consuming log2(bsize) levels

void CipherLRPvals::fullReplicate(Ciphertext*& res, Ciphertext encData, uint64_t** poly, long* rot, long subblocksize, long nstep){
    
    bool ntepIsOdd = (bool)(nstep % 2);
    
    long blocksize = subblocksize/2;
    Ciphertext* tmp = new Ciphertext[blocksize];    //! maximal size for storing ctxts
    
    // "+--------------------------------------------------------+"
    //!  0st layer : E(0,..., bsize/2 - 1) / E(bsize/2,..., bsize - 1)
    //! (nstep = even): tmp (updated)
    //! (nstep = odd): res (updated)
    // "+--------------------------------------------------------+"
    if(ntepIsOdd){
        res[0] = encData;
        scheme.multByPolyAndEqual(res[0], poly[0]);
        scheme.reScaleByAndEqual(res[0], 1);
        res[1] = scheme.modDownTo(encData, res[0].l);
        scheme.subAndEqual(res[1], res[0]);
        for(long i = 0; i < 2; ++i){    //! = 8 / 4
            Ciphertext ctemp = extscheme.leftRotateFast(res[i], rot[0]);
    ...
    //! (nstep = even): tmp (updated) -> res (l = 1), tmp, ..., res
    //! (nstep = odd): res (updated)  -> tmp (l = 1), res (l = 2), ..., res
    // "+--------------------------------------------------------+"
    
    for(long l = 1; l < nstep; ++l){
        long i1 = (1 << (l));
        bool lIsOdd = (bool) (l % 2);
        if ((ntepIsOdd + lIsOdd)==1){
            for(long i = 0; i < i1; ++i){
                long j = (i << 1);
                res[j] = tmp[i];
                scheme.multByPolyAndEqual(res[j], poly[l]);
                scheme.reScaleByAndEqual(res[j], 1);
                res[j + 1] = scheme.modDownTo(tmp[i], res[j].l); //! E(2,3)
                scheme.subAndEqual(res[j + 1], res[j]);
            }
            for(long i = 0; i < (i1 << 1); ++i){    // 8/2
                Ciphertext ctemp = extscheme.leftRotateFast(res[i], rot[l]);
                scheme.addAndEqual(res[i], ctemp);
            }
        }
        else{
            for(long i = 0; i < i1; ++i){
                long j = (i << 1);
                tmp[j] = res[i];
                scheme.multByPolyAndEqual(tmp[j], poly[l]);
                scheme.reScaleByAndEqual(tmp[j], 1);
                tmp[j + 1] = scheme.modDownTo(res[i], tmp[j].l); //! E(2,3)
                scheme.subAndEqual(tmp[j + 1], tmp[j]);
            }
            for(long i = 0; i < (i1 << 1); ++i){    // 8/2
                Ciphertext ctemp = extscheme.leftRotateFast(tmp[i], rot[l]);
                scheme.addAndEqual(tmp[i], ctemp);
            }
        }
...
void CipherLRPvals::sparseReplicate(Ciphertext*& res, Ciphertext encData, uint64_t** poly, long nslots, long nvals, long* rot, long subblocksize, long nstep){
    ...
    // "+------------------------------------+"
    double unitsize = (double) blocksize;
    long nctxt = (long) ceil((double)nvals/unitsize);
    ...
    // "+------------------------------------+"
    for(long l = 1; l < nstep; ++l){
        unitsize = (unitsize/2.0);
        nctxt = (long) ceil((double)nvals/unitsize);
        ...
        if ((ntepIsOdd + lIsOdd)==1){
            for(long i = 0; i < nvals1; ++i){
                long i1 = (i << 1);
                res[i1] = tmp[i];
                scheme.multByPolyAndEqual(res[i1], poly[l]);
            ...
            if(nctxt % 2){
                ...
                scheme.multByPolyAndEqual(res[nctxt - 1], poly[l]);
            ...
            for(long i = 0; i < nvals1; ++i){
                long i1 = (i << 1);
                tmp[i1] = res[i];
                scheme.multByPolyAndEqual(tmp[i1], poly[l]);
            ...
            if(nctxt % 2){
                ...
                scheme.multByPolyAndEqual(tmp[nctxt - 1], poly[l]);
