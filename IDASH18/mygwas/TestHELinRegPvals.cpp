...
void TestHEPvals::testFastHELinReg(double*& zScore, double*& pVals, double* yData, double** xData, double** sData, long factorDim, long sampleDim, long nsnp, vector<string> snptag, string filename){
    ...
    cipherPvals.encryptSData(encYSData, encSXData, yData, xData, sData, factorDim, sampleDim, nsnp, nencsnp, nslots, Slvl, YSlvl, SXlvl);
    ...
    cout << "Freshly Ciphertexts size: " << (sizeYX + sizecov  + sizeYS + sizeSX) << "(MB)" << endl;
    ...
    for(long j = first; j < last; ++j){
        ...
        encSnorm[j] = scheme.modDownTo(encSX[0][j], encDet.l);  //! L - 2
        extscheme.multAndEqual(encSnorm[j], encDet);
