...
result=read.table("FastHELinReg_Pvals.txt", header = F)
...
tp = length(which(pval_he[which(pval_sp<=threshold)]<=threshold))/length(which(pval_sp<=threshold))


#type i error/false positive
print("type i error/false positive rate")
fp = length(which(pval_he[which(pval_sp>threshold)]<=threshold))/(length(which(pval_he[which(pval_sp>threshold)]<=threshold)) + length(which(pval_he[which(pval_sp>threshold)]>threshold)))


#type ii error/false negative
print("type ii error/false negative rate")
fn = length(which(pval_he[which(pval_sp<=threshold)]>threshold))/length(which(pval_sp<=threshold))

#true negative
print("true negative rate")
tn = length(which(pval_he[which(pval_sp>threshold)]>threshold))/length(which(pval_sp>threshold))

precision = length(which(pval_he[which(pval_sp<=threshold)]<=threshold))/length(which(pval_he<=threshold))
f1score = 2 * tp *  precision / (precision+tp)

qp<-qplot(pval_he, pval_sp, main=paste("F1 score",toString(round(f1score,digits = 3)),sep = "="), log="xy", xlab="Secure Semi-Parallel Logistic Regression", ylab="Gold standard")
